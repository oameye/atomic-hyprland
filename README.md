...
The hyprwm stack itself comes from the [`lionheartp/Hyprland`](https://copr.fedorainfracloud.org/coprs/lionheartp/Hyprland/) COPR, installed in [`packages.sh`](./build_files/packages.sh). That COPR packages tagged **stable** Hyprland releases (plus `uwsm` and the uwsm session file), so the compositor is pinned to a released version rather than chasing git master, and the whole session stack comes from one coherent source. Everything not packaged for Fedora (walker, elephant, wiremix, the share picker) is still source-built in [`source-builds.sh`](./build_files/source-builds.sh).
