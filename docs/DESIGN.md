...
- `ujust update` is overlaid at `files/usr/share/ublue-os/just/10-update.just` to keep manual updates command-by-command in the terminal while adding `brew update` and `brew upgrade` to the stock `rpm-ostree` / `flatpak` / `distrobox` flow. Automatic updates still run separately through `uupd.timer`.
